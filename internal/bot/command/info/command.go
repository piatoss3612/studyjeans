...
func NewInfoCommand(svc service.Service, pub msgqueue.Publisher, cache cache.Cache, sugar *zap.SugaredLogger) command.Command {
	return &infoCommand{
		svc:   svc,
		pub:   pub,
		cache: cache,
...
func (ic *infoCommand) myStudyInfoCmdHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if err != nil {
		go func() {
			...
			go ic.publishEvent(evt)
		}()
		ic.sugar.Errorw(err.Error(), "event", i.ApplicationCommandData().Name)
...
func (ic *infoCommand) studyInfoCmdHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	fn := func(s *discordgo.Session, i *discordgo.InteractionCreate) error {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
...
func (ic *infoCommand) studyRoundInfoCmdHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	fn := func(s *discordgo.Session, i *discordgo.InteractionCreate) error {
		var user *discordgo.User
...
func (ic *infoCommand) speakerInfoSelectMenuHandler(s *discordgo.Session, i *discordgo.InteractionCreate) {
	fn := func(s *discordgo.Session, i *discordgo.InteractionCreate) error {
		var user *discordgo.User
...
func (ic *infoCommand) publishEvent(evt event.Event) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	for i := 0; i < 10; i++ {
		err := ic.pub.Publish(ctx, evt.Topic(), evt)
		if err != nil {
			ic.sugar.Errorw(err.Error(), "event", "publish event", "topic", evt.Topic(), "try", i+1)
			continue
		}
		return
	}
