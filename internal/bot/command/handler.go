...
func (h *handler) Handle(name string, s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if err := fn(s, i); err != nil {
		...
		go func() {
			evt, err := cerrors.NewEvent(cerrors.EventTopicError, fmt.Sprintf("%s: %s", name, err.Error()))
			if err != nil {
				h.sugar.Errorw("failed to create error event", "error", err.Error())
				return
			}

			go h.publishError(evt)
		}()
...
func (h *handler) publishError(evt cerrors.Event) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		default:
			err := h.pub.Publish(ctx, evt.Topic.String(), evt)
			if err != nil {
